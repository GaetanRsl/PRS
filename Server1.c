            ...
            * */
            ...
            if (pid > 0){
                ...
                **/
                ...
                while((fs_block_sz = fread(sdbuf, sizeof(char), 1494,fs)) > 0 ){
                    //printf("avant\n");
                    memcpy(tableau_seq[nbPaquet].seq, sdbuf,1494);
                ...
                * */
                ...
                while(last_ack < nbPaquet){
                    ...
                    for(int i = start_window; i<=end_window; i++){
                        ...
                        bzero(buffer_seq, 6);
                        bzero(buffer_data_seq, 1500);

                        //Ajout data + seq dans buffer
                        sprintf(buffer_seq,"%6d", i);
                        memcpy(buffer_data_seq, buffer_seq, 6);
                        memcpy(buffer_data_seq + 6,tableau_seq[i].seq, 1494);
                        
                        if(i>last_sent){
                            sendto(socketUDP, buffer_data_seq, 1500, 0, (struct sockaddr *)&client_UDP, b);
                    ...
                    if(FD_ISSET(socketUDP, &fd_conn)!=0){
                        ...
                        if (seq > last_ack){
                            ...
                            memcpy(buffer_data_seq, buffer_seq, 6);
                            memcpy(buffer_data_seq + 6,tableau_seq[seq+1].seq, 1494);
                        ...
                        **/    
                        ...
                        memcpy(buffer_data_seq, buffer_seq, 6);
                        memcpy(buffer_data_seq + 6,tableau_seq[last_ack+1].seq, 1494);
                        sendto(socketUDP, buffer_data_seq, 1500, 0, (struct sockaddr *)&client_UDP, b);
